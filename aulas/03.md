...
Usamos códigos de estado para informar al cliente el resultado de las operaciones en el servidor, como si un dato fue creado, encontrado, actualizado o eliminado con éxito. Por eso invertiremos un poco más de tiempo aquí.

Los códigos a los que debemos prestar atención para responder correctamente las solicitudes. Los casos de éxito incluyen:
...
Para facilitar el aprendizaje, sugiero dividir la creación de nuevos endpoints en tres etapas:
...
El modelo de datos es una parte fundamental, donde consideramos tanto los datos recibidos del cliente como los datos que serán devueltos a él. Este enfoque asegura una comunicación eficaz y clara.
...
Para aplicar un modelo a la respuesta del endpoint, tenemos que pasar el modelo al parámetro `response_model`, como hicimos en la [clase pasada](02.md#integrando-o-pydantic-com-o-fastapi){:target="_blank"}:
...
Vemos que pydantic desempeña un papel bastante importante en el funcionamiento de la API. Porque puede “bloquear” el request antes de que sea expuesto a nuestra función de endpoint. Evitando que diversos casos extraños sean cubiertos de forma transparente. Tanto en relación a los tipos de los campos, como en relación a los campos que deberían ser enviados, pero no lo fueron.
...
De esta forma, el campo esperará no solamente una string como antes, sino una dirección de email válida.
...
??? info "Si las fixtures son una novedad para ti"
	Existe una live de Python donde discutimos específicamente sobre fixtures
	![type:video](https://www.youtube.com/embed/sidi9Z_IkLU)

	[:fontawesome-brands-youtube: Link directo ](https://youtu.be/sidi9Z_IkLU){ .md-button }
...
## Complementario / Para la próxima clase

Para la próxima clase, en caso de que no tengas ninguna familiaridad con SQLAlchemy o con Alembic, recomiendo que veas estas lives para prepararte y nivelar un poco el conocimiento sobre estas herramientas:

- [SQLAlchemy: conceptos básicos, una introducción a la versión 2 | Live de Python #258](https://youtu.be/t4C1c62Z4Ag){:target="_blank"}
- [Migraciones, bases de datos evolutivas (Alembic y SQLAlchemy) | Live de Python #211](https://youtu.be/yQtqkq9UkDA){:target="_blank"}

Otro recurso que usaremos en la próxima clase y puede ayudarte a saber un poco, son las variables de entorno. Tema abordado en:

- [Variables de entorno, dotenv, constantes y configuraciones | Live de Python #207](https://youtu.be/DiiKff1z2Yw){:target="_blank"}
